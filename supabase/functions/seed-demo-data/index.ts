...
serve(async (req) => {
  ...
  try {
    ...
    // Delete existing demo community posts first
    await supabaseClient
      .from('community_posts')
      .delete()
      .like('content', '%DEMO_SEED%');

    // Get or create official "Sons of Legion" profile for announcements
    // We'll use the first Legionnaire or just a stable user_id
    const officialUserId = profiles.find(p => p.tier === 'Legionnaires')?.user_id || profiles[0]?.user_id;

    // Create announcements from official account
    const announcementPosts = [];
    for (let i = 0; i < announcementContent.length; i++) {
      const announcement = announcementContent[i];
      const daysAgo = i * 5 + randomBetween(1, 4); // Spread over time
    ...
    // Create Legion Speaks posts from different staff personas
    // Use different profiles for each staff member
    const shuffledProfiles = shuffleArray(profiles.filter(p => p.tier === 'Legionnaires' || p.tier === 'Outlaws'));
    const legionSpeaksPosts = [];
    
    for (let i = 0; i < legionSpeaksContent.length; i++) {
      const post = legionSpeaksContent[i];
      const authorProfile = shuffledProfiles[i % shuffledProfiles.length];
      const daysAgo = i * 3 + randomBetween(1, 3);
      const createdAt = randomDate(daysAgo);
      
      legionSpeaksPosts.push({
        user_id: authorProfile?.user_id || officialUserId,
        content: post.content,
        category: 'legion-speaks',
    ...
    // Create member intro posts from random actual profiles
    const introProfiles = shuffleArray([...profiles]).slice(0, Math.min(introTemplates.length, profiles.length));
    const introPosts = [];
    
    for (let i = 0; i < introProfiles.length && i < introTemplates.length; i++) {
      const profile = introProfiles[i];
    ...
    // Get all inserted post IDs for reactions and comments
    ...
    if (allInsertedPosts.length > 0) {
      ...
      for (const post of allInsertedPosts) {
        const numReactions = randomBetween(3, 20);
      ...
      // Insert reactions in batches to avoid hitting limits
      const batchSize = 100;
    ...
    return new Response(
      JSON.stringify({ 
        ...
        message: 'Demo data seeded successfully with comprehensive engagement and community data'
      }),
